...
#### Scenario: Default backup [hosts: macm5, rog]
...
#### Scenario: Supported target and inspection [hosts: rog, thinkcentre, t14, macm5]
...
#### Scenario: Required state is archived [hosts: macm5, rog]
...
#### Scenario: Regenerable or live state is absent [hosts: macm5, rog]
...
#### Scenario: Snapshot validation fails closed [hosts: macm5, rog]
...
#### Scenario: Portable publication succeeds [hosts: macm5, rog]
...
#### Scenario: Restore preserves database indirection [hosts: rog, thinkcentre, t14, macm5]
...
#### Scenario: Corrupt archive or database fails [hosts: rog, thinkcentre, t14, macm5]
...
#### Scenario: Operator follows recovery guidance [hosts: rog, thinkcentre, t14, macm5]
...
Acceptance MUST require clean `/bin/sh -n` over both embedded payloads plus `go test ./...`, `format-nix`, and `nix flake check --no-build`; a real macm5 archive containing `.db.snapshot` and `.claude.json` but no OpenCode `bin/`, `log/`, `snapshot/`, `node_modules`, `.config/opencode`, or live `*.db`; and passing snapshot integrity and `sha256 -c` checks.

#### Scenario: Release evidence passes [hosts: macm5, rog]
