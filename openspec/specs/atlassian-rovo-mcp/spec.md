...
Define the official Atlassian Rovo MCP registration for macm5, the removal contract for the
...
#### Scenario: Evaluated macm5 entry has the official shape [hosts: macm5]

- GIVEN the evaluated `homeConfigurations.macm5` MCP inventory
...
#### Scenario: No local command remains [hosts: macm5]

- GIVEN the evaluated macm5 MCP inventory
- WHEN the `atlassian` entry is inspected
...
#### Scenario: Wrapper and import are gone [hosts: macm5]
...
#### Scenario: Secret declarations are pruned [hosts: macm5]

- GIVEN the evaluated `homeConfigurations.macm5` sops secrets
...
#### Scenario: Encrypted artifacts untouched [hosts: macm5]
...
`docs/atlassian-rovo-mcp.md` MUST document: the first-time OAuth 2.1 login on macm5 with the
...
#### Scenario: A user can complete migration [hosts: macm5]

- GIVEN the runbook
- WHEN the user follows it on macm5 after rebuild
- THEN OAuth login, verification, and uninstall steps are all present and ordered

### Requirement: Cross-tool propagation

The official remote entry MUST reach both OpenCode and Claude Code on macm5 through the existing
shared consumers, with Claude Code receiving `{ type = "http"; url = "https://mcp.atlassian.com/v2/mcp" }`.

#### Scenario: Consumers require no edits [hosts: macm5]
...
#### Scenario: Scope stays darwin-only and additive-minimal [hosts: macm5]
