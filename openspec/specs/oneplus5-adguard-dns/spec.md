# oneplus5-adguard-dns Specification

## Purpose

Give smart TVs per-device DNS-query visibility without moving DHCP away from the Xiaomi router, by operating a reversible, log-first AdGuard Home service on the always-on OnePlus 5 for manually enrolled TVs only. The phone serves LAN DNS on the stable address `172.16.0.12`, with opt-in Hagezi filtering and a documented TV-first rollback, while leaving NixOS hosts and router DHCP unchanged.
