# Atlassian Rovo MCP Specification (mact2)
...
#### Scenario: Evaluated mact2 entry has the official shape [mact2]
...
#### Scenario: No local command remains [mact2]
...
#### Scenario: Wrapper and import are gone [mact2]
...
#### Scenario: Secret declarations are pruned [mact2]
...
#### Scenario: Encrypted artifacts untouched [mact2]
...
#### Scenario: A user can complete migration [mact2]
...
#### Scenario: Consumers require no edits [mact2]
...
#### Scenario: Scope stays darwin-only and additive-minimal [mact2]
...
- `openspec/changes/atlassian-rovo-mcp-mact2/proposal.md` — approved scope and capability.
- `openspec/changes/atlassian-rovo-mcp-mact2/exploration.md` — verified current stack, official
  server research, auth and secrets findings.
- `darwin/home/opencode/mcps-extra.nix` — darwin extra MCP inventory.
- `shared/opencode/runtime-config.nix`, `shared/claude-code.nix` — remote consumers (read-only).
- https://github.com/atlassian/atlassian-mcp-server — official endpoint and auth contract.
