...
|------|------------|------------------|-----------|
| rog | HTTPS (token via GH_TOKEN) | "Redacted Name" / personal@example.com | `github/pat` from shared/passwords.yaml |
| thinkcentre | HTTPS (token via GH_TOKEN) | "Redacted Name" / personal@example.com | `github/pat` from shared/passwords.yaml |
| t14 | HTTPS (token via GH_TOKEN) | "Redacted Name" / personal@example.com | `github/pat` from shared/passwords.yaml |
| mact2 | SSH (id_ed25519_github) + token | work@example.com | `github/token` from atlassian.yaml |
...
1. **Linux (home-linux/git.nix)**: Add conditional include for work repos
   - Default identity stays as-is (glats/Redacted Name)
