...
2. Replace `"Juan Cuzmar"` with `identities.glats.name` and `"jcuzmar@protonmail.com"` with `identities.glats.email`
...
| AC | Criterion | Method |
|----|-----------|--------|
| AC-1 | Linux ~/dev/* shows glats identity | Manual: `cd ~/dev/* && git config user.name` → "Juan Cuzmar" |
...
| AC-4 | macOS ~/Personal/* shows glats identity | Manual: `cd ~/Personal/* && git config user.name` → "Juan Cuzmar" |
