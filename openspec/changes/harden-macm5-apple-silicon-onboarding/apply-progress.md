...
- Work unit: remote-safe macm5 link identity staging
...
| Runtime harness command/scenario and exact result | Native macm5 activation is not run from Linux; platform/user evaluations returned `aarch64-darwin`/`juan` and runtime evidence remains pending. No secret values were read. |
...
- Native acceptance and evidence remain pending: tasks 1.1-1.3, 3.1-3.3, and 4.1-4.2.
- Remote-safe task 2.2 is implemented; encrypted files still require the documented admin-only `uuid_macm5` edit and `sops updatekeys` rotation.
