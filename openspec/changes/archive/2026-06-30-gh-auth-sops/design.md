...
| `home-linux/git.nix` | Modify | Add `settings.user.name = "Juan Cuzmar"` and `settings.user.email = "jcuzmar@protonmail.com"` |
...

{
  programs.git = {
    ...
    settings = {
      ...
      core.pager = "delta";
      delta.enable = true;
      user.name = "Juan Cuzmar";
      user.email = "jcuzmar@protonmail.com";
    };
  };
}
```
...
|-----------------|---------------------|
| Git identity set | `git config user.name` → `Juan Cuzmar` |
| Git identity set | `git config user.email` → `jcuzmar@protonmail.com` |
