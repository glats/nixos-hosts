...
The system SHALL display the current keyboard layout name in the waybar right module group. The display MUST update within 5 seconds of layout changes.
...
- THEN within 5 seconds the waybar display SHALL update to show "latam"
...
- THEN the system SHALL execute `~/.local/share/omarchy/bin/kb-toggle.sh`
- AND the layout SHALL change to "latam"
- AND the waybar display SHALL update to show "latam" within 5 seconds
...
- AND the waybar display SHALL update to show "es" within 5 seconds
...
- Hyprland MUST be configured with xkb layout groups for es and latam
- Waybar MUST be running on the t14 host

## Constraints

- This module is specific to the t14 host
- The module uses polling (5-second interval) rather than event-driven updates
