...
### Requirement: Root-Managed Full-Tunnel Routing

`mact2` MUST run the TUN client as a root LaunchDaemon. `tunnel.mode` MUST default to `full`: rules MUST send `ip_is_private` direct first, then `tunnel.directDomains` and `tunnel.directCidrs` direct, then select `tunnel-out` as final. `tunnel.mode = "scoped"` MUST instead tunnel only `chatgpt.com` and `auth.openai.com` and select direct as final.
...
- GIVEN full mode and a configured LAN `tunnel.directDomains` endpoint
- WHEN `sing-box check -c <rendered-config>`, `launchctl print system/sing-box-tunnel`, an IP-echo HTTPS request, and the LAN request run
- THEN the echo service reports rog's egress IP
- AND logs show the LAN request selected `direct`, not `tunnel-out`
...
- THEN only `chatgpt.com` and `auth.openai.com` select `tunnel-out`
