...
- **engram**: `mem_get_observation` the Engram topics `sdd/{change-name}/proposal`, `sdd/{change-name}/spec`, `sdd/{change-name}/design`, `sdd/{change-name}/tasks` (all required — keep tasks ID for updates). Mark tasks complete via `mem_update(id: {tasks-observation-id}, content: "...")`. Save progress with `mem_save(topic_key="sdd/{change-name}/apply-progress", ...)`. Do NOT read or write filesystem paths under `sdd/` — use `openspec/` for filesystem artifact writes.
