...
- **engram**: `mem_get_observation` the Engram topic `sdd/{change-name}/proposal` (required). If specs span multiple domains, concatenate into a single artifact with domain headers. Save with `mem_save(topic_key="sdd/{change-name}/spec", ...)`. Do NOT read or write filesystem paths under `sdd/` — use `openspec/` for filesystem artifact writes.
