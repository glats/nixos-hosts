...
- **engram**: `mem_get_observation` the Engram topics `sdd/{change-name}/explore` (optional) and `sdd-init/{project}` (optional). Save the new artifact with `mem_save(topic_key="sdd/{change-name}/proposal", ...)`. Do NOT read or write filesystem paths under `sdd/` — use `openspec/` for filesystem artifact writes.
